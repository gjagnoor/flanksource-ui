...
export const getIncidentHistory = async (incidentID: string) => {
  const res = await IncidentCommander.get<IncidentHistory[]>(
    `/incident_histories?incident_id=eq.${incidentID}&select=*,incident:incidents(*),evidence:evidences(*),hypothesis:hypotheses(*),responder:responders(*,person:person_id(id,name,avatar)),comment:comments(*),created_by(id,name,avatar))&order=created_at.desc`
  );
  return res.data;
};
